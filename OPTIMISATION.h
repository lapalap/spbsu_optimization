...
protected:
    ...
    std::vector<std::vector<double>> _journal;
...
public:
    ...
    void update_func(FUNC* function);
    void update_area(AREA* a);
    void update_stop(STOP* stop_method);
    std::vector<std::vector<double>> get_journal();
    void clear();
};
...
private:
    ...
    double p, eps;
public:
    Coordinate_wise(FUNC* function, AREA* a, STOP* stop_method, const double N_samples = 100000, const double eps = 0.001);
    ...
    void update_epsilon(double newEpsilon);
...
private:
	...
	double eps;
public:
    Random_search(FUNC* function, AREA* a,  STOP* stop_method, const double p = 0.8, const double eps = 0.0001);
	const std::vector<double> optimise(const std::vector<double> start);
	const std::vector<double> get_arg();
	const double get_val();
    const unsigned int get_iterations();
    void update_prob(double prob);
